    ...
    try:
        ...
        if "response" in data and isinstance(data["response"], list):
            for g in data["response"]:
                ...
                game_id = str(g["fixture"]["id"]) if "fixture" in g else f"{home}_{away}"
...
def fetch_player_stats(team):
    return [
        {"name":"QB1","pass_yards":280,"rush_yards":30,"rec_yards":0,"img":"https://a.espncdn.com/combiner/i?img=/i/headshots/nfl/players/full/4032978.png&w=50&h=50"},
        {"name":"RB1","pass_yards":0,"rush_yards":100,"rec_yards":20,"img":"https://a.espncdn.com/combiner/i?img=/i/headshots/nfl/players/full/4032979.png&w=50&h=50"},
        {"name":"WR1","pass_yards":0,"rush_yards":10,"rec_yards":70,"img":"https://a.espncdn.com/combiner/i?img=/i/headshots/nfl/players/full/4032980.png&w=50&h=50"}
    ]
...
def calculate_props(players, opponent_def):
    ...
    for p in players:
        ...
        props.append({
            ...
            "line":p['pass_yards'],
            "odds":1.9,
            "reason":"Based on recent performance vs opponent",
            "EV":round(ev_pass,2),
            "img": p.get("img","")
        })
        props.append({
...
# --- Bets Page ---
    ...
    if sport=="CFB":
        ...
        # Simulated opponent defense
        opponent_def = {"vs_pass":70, "vs_rush":80, "vs_rec":75}
        
        home_df = calculate_props(home_players, opponent_def)
        away_df = calculate_props(away_players, opponent_def)
        
        st.markdown("**Home Team Players**")
        for _, row in home_df.iterrows():
            st.markdown(f"**{row['player']} - {row['prop']}**: Line {row['line']}, Odds {row['odds']}, EV {row['EV']}")
            st.image(row['img'], width=50)
